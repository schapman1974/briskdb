...
async fn unchanged_odm_applications_use_real_pymongo_before_and_after_restart() {
    ...
    let report_root = std::env::var_os("BRISKDB_MONGO_ODM_REPORT_DIR")
        .map(PathBuf::from)
    ...
    for phase in ["initial", "reopened"] {
        ...
        let output = tokio::task::spawn_blocking(move || run_client(uri, phase, report)).await;
...
fn run_client(uri: String, phase: &str, report: PathBuf) -> Output {
    ...
    let mut child = Command::new(python)
        ...
        .spawn()
        .expect("launch pinned ODM runner");
    ...
    let status = loop {
        match child.try_wait() {
            ...
            outcome => {
                let _ = child.kill();
                let _ = child.wait();
                panic!("ODM runner did not finish within its bound: {outcome:?}");
            }
        }
    };
