...
def membership_index_smoke(uri, reopened):
    with pymongo.MongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_membership.sync_items
        query = {"a": {"$in": [Int64(1), 2, 1.0]}, "enabled": True}
        if not reopened:
            collection.insert_many([{"_id": i, "a": [i % 4, (i + 1) % 4, i % 4],
                                     "enabled": i % 2 == 0, "rank": i, "hits": 0} for i in range(32)])
            collection.create_index("a")
            assert [row["_id"] for row in collection.find(query).batch_size(1)] == list(range(0, 32, 2))
            result = collection.update_many(query, {"$inc": {"hits": 1}})
            assert (result.matched_count, result.modified_count) == (16, 16)
            assert collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("rank", -1)],
                                                 return_document=True)["hits"] == 2
            assert collection.replace_one(dict(query, _id=2), {"a": [1, 2], "enabled": True,
                                                                "rank": 2, "hits": 3}).modified_count == 1
            assert collection.find_one_and_replace(dict(query, _id=4), {"a": [2, 1], "enabled": True,
                                                                       "rank": 4, "hits": 4})["hits"] == 1
            assert collection.delete_one(dict(query, _id=6)).deleted_count == 1
            assert collection.find_one_and_delete(query, sort=[("_id", -1)])["_id"] == 30
            assert collection.update_one({"_id": 100, "a": {"$in": [7, 8]}},
                                         {"$set": {"a": 7, "enabled": False, "hits": 10}}, upsert=True).upserted_id == 100
        expected = [i for i in range(0, 32, 2) if i not in (6, 30)]
        rows = list(collection.find(query).sort("_id", 1).batch_size(1))
        assert [row["_id"] for row in rows] == expected
        assert [row["hits"] for row in rows] == [3 if i == 2 else 4 if i == 4 else 1 for i in expected]
        assert [row["_id"] for row in collection.find(query).sort("rank", 1).skip(1).limit(3)] == [2, 4, 8]
        assert collection.count_documents(query) == 14
        assert sorted(collection.distinct("a", query)) == [0, 1, 2, 3]
        assert collection.count_documents({}) == 31
        assert collection.find_one({"_id": 100})["hits"] == 10


async def async_membership_index_smoke(uri, reopened):
    async with pymongo.AsyncMongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_membership.async_items
        query = {"a": {"$in": [Int64(1), 2, 1.0]}, "b": {"$in": [1, 2]}}
        selected = [i for i in range(32) if i % 4 != 3 and i % 3 != 0]
        if not reopened:
            await collection.insert_many([{"_id": i, "a": [i % 4, (i + 1) % 4],
                                            "b": i % 3, "hits": 0} for i in range(32)])
            await collection.create_index([("a", 1), ("b", -1)])
            ...
            assert (await collection.delete_one(query)).deleted_count == 1
            assert (await collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("_id", -1)],
                                                         return_document=True))["hits"] == 2
        expected = selected[1:]
        rows = await collection.find(query).sort("_id", 1).batch_size(1).to_list()
        assert [row["_id"] for row in rows] == expected
        assert [row["hits"] for row in rows] == [2 if i == expected[-1] else 1 for i in expected]
        assert await collection.count_documents(query) == len(expected)
        assert sorted(await collection.distinct("b", query)) == [1, 2]
        assert await collection.count_documents({}) == 31
...
def absence_index_smoke(uri, reopened):
    with pymongo.MongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_absence.sync_items
        query = {"a": {"$exists": False}}
        if not reopened:
            records = []
            for i in range(40):
                row = {"_id": i, "rank": i, "hits": 0}
                if i % 5:
                    row["a"] = [None, [], [1, 2, 1], {"nested": i}][i % 5 - 1]
                records.append(row)
            collection.insert_many(records)
            collection.create_index("a")
            assert [row["_id"] for row in collection.find(query).batch_size(1)] == list(range(0, 40, 5))
            result = collection.update_many(query, {"$inc": {"hits": 1}})
            assert (result.matched_count, result.modified_count) == (8, 8)
            assert collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("rank", -1)], return_document=True)["hits"] == 2
            assert collection.replace_one(dict(query, _id=5), {"a": 1, "rank": 5, "hits": 3}).modified_count == 1
            assert collection.find_one_and_replace(dict(query, _id=10), {"a": None, "rank": 10, "hits": 4})["hits"] == 1
            assert collection.delete_one(dict(query, _id=0)).deleted_count == 1
            assert collection.find_one_and_delete(query, sort=[("rank", -1)])["_id"] == 35
            assert collection.update_one(dict(query, _id=100), {"$set": {"hits": 10}}, upsert=True).upserted_id == 100
        rows = list(collection.find(query).sort("_id", 1).batch_size(1))
        assert [row["_id"] for row in rows] == [15, 20, 25, 30, 100]
        assert [row["hits"] for row in rows] == [1, 1, 1, 1, 10]
        assert collection.count_documents(query) == 5
        assert sorted(collection.distinct("hits", query)) == [1, 10]
        assert collection.count_documents({}) == 39
        assert collection.find_one({"_id": 10})["a"] is None
        assert collection.find_one(dict(query, _id=10)) is None


async def async_absence_index_smoke(uri, reopened):
    async with pymongo.AsyncMongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_absence.async_items
        query = {"a": {"$exists": False}, "b": {"$in": [Int64(1), 2.0]}}
        selected = [i for i in range(40) if i % 5 == 0 and i % 3 != 2]
        if not reopened:
            records = []
            for i in range(40):
                row = {"_id": i, "b": i % 3 + 1, "hits": 0}
                if i % 5:
                    row["a"] = [None, [], [1, 2, 1], {"nested": i}][i % 5 - 1]
                records.append(row)
            await collection.insert_many(records)
            ...
            assert (result.matched_count, result.modified_count) == (len(selected), len(selected))
            assert (await collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("_id", -1)], return_document=True))["hits"] == 2
            result = await collection.update_many(query, {"$set": {"a": [1, 2]}})
            assert (result.matched_count, result.modified_count) == (len(selected), len(selected))
        assert await collection.count_documents(query) == 0
        assert await collection.count_documents({"a": {"$exists": False}}) == 3
        rows = await collection.find({}).sort("_id", 1).batch_size(2).to_list()
        assert [row["_id"] for row in rows] == list(range(40))
        assert [row["hits"] for row in rows] == [2 if i == selected[-1] else 1 if i in selected else 0 for i in range(40)]
        assert all(row["a"] == [1, 2] for row in rows if row["_id"] in selected)


def logical_index_fixture(compound):
    records = []
    for i in range(42):
        row = {"_id": i, "rank": i, "b": i % 3 + 1, "hits": 0}
        if i % 7:
            row["a"] = [None, 1, True, [1, 2, 1], 9, {"nested": i}][i % 7 - 1]
        records.append(row)
    query = {"$or": [{"a": 1}, {"a": {"$in": [Int64(2), 1.0]}}]}
    if compound:
        query["b"] = {"$in": [Int64(1), 2.0]}
    selected = [i for i in range(42) if i % 7 in (2, 4) and (not compound or i % 3 != 2)]
    return records, query, selected


def logical_index_smoke(uri, reopened):
    records, query, selected = logical_index_fixture(False)
    with pymongo.MongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_logical_index.sync_items
        if not reopened:
            collection.insert_many(records)
            collection.create_index("a")
            ...
            assert (result.matched_count, result.modified_count) == (len(selected), len(selected))
            assert collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("rank", -1)], return_document=True)["hits"] == 2
            assert collection.replace_one(dict(query, _id=selected[0]), {"a": 9, "hits": 3}).modified_count == 1
            assert collection.find_one_and_delete(query, sort=[("rank", -1)])["_id"] == selected[-1]
            assert collection.update_one(dict(query, _id=100), {"$set": {"a": 1, "hits": 10}}, upsert=True).upserted_id == 100
        rows = list(collection.find(query).sort("_id", 1).batch_size(1))
        ...
        assert sorted(collection.distinct("hits", query)) == [1, 10]
        assert collection.count_documents({}) == 42
        assert collection.find_one({"_id": selected[0]})["a"] == 9


async def async_logical_index_smoke(uri, reopened):
    records, query, selected = logical_index_fixture(True)
    async with pymongo.AsyncMongoClient(uri, serverSelectionTimeoutMS=3000, socketTimeoutMS=3000) as client:
        collection = client.wire_logical_index.async_items
        if not reopened:
            await collection.insert_many(records)
            await collection.create_index([("a", 1), ("b", -1)], sparse=True)
            assert [row["_id"] async for row in collection.find(query).batch_size(1)] == selected
            result = await collection.update_many(query, {"$inc": {"hits": 1}})
            assert (result.matched_count, result.modified_count) == (len(selected), len(selected))
            assert (await collection.find_one_and_update(query, {"$inc": {"hits": 1}}, sort=[("rank", -1)], return_document=True))["hits"] == 2
            assert (await collection.replace_one(dict(query, _id=selected[0]), {"a": 9, "hits": 3})).modified_count == 1
            assert (await collection.find_one_and_delete(query, sort=[("rank", -1)]))["_id"] == selected[-1]
            assert (await collection.update_one(dict(query, _id=100), {"$set": {"a": 1, "b": 1, "hits": 10}}, upsert=True)).upserted_id == 100
        rows = await collection.find(query).sort("_id", 1).batch_size(1).to_list()
        assert [row["_id"] for row in rows] == selected[1:-1] + [100]
        assert [row["hits"] for row in rows] == [1] * (len(selected) - 2) + [10]
        assert await collection.count_documents(query) == len(selected) - 1
        assert sorted(await collection.distinct("hits", query)) == [1, 10]
        assert await collection.count_documents({}) == 42
...
if __name__ == "__main__":
    ...
    absence_index_smoke(sys.argv[1], reopened)
