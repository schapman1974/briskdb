        ...
        with tempfile.TemporaryDirectory() as root:
            with briskdb.open(root, shards=4, documents=True) as database:
                with database.session() as session:
                    ...
                    self.assertEqual(skipped["documents"], [])
                    self.assertEqual(skipped["read_stats"]["source_matches"], 1)
                    filtered = session.aggregate(DATABASE, COLLECTION, [{"$project": {"v": 1}}, {"$match": {"v": 1}}], execution_stats=True)
