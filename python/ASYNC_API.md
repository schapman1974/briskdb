...
`return_document` before/after selection. `update_one` supports `$set`/`$unset`/`$min`/`$max`
...
`find_one_and_update` supports the same four operators and preflights the returned
