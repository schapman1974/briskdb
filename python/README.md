...
`$set`/`$unset`/`$min`/`$max`/`$pop`/`$rename`/`$addToSet`/`$pullAll`/`$push`/`$pull` updates, and retained cursors through synchronous and asyncio
...
query predicates to array members without creating missing fields. Continue a
