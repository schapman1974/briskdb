...
`$set`/`$unset`/`$min`/`$max`/`$pop`/`$rename`/`$addToSet`/`$pullAll`/`$push` updates, and retained cursors through synchronous and asyncio
...
whole-BSON/document-field sorting. Continue a
