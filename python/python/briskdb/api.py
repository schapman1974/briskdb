...
class AsyncSession:
    ...
    async def list_database_names(
        self, filter: Any = None, *, request_id: Optional[UUID] = None,
        timeout_ms: Optional[int] = None, cancellation: Optional[CancellationToken] = None,
        max_result_rows: Optional[int] = None, max_result_bytes: Optional[int] = None,
    ) -> dict[str, Any]:
        return await _cancelable_call(
            self._session.list_database_names, filter, request_id=request_id,
            timeout_ms=timeout_ms, cancellation=cancellation,
            max_result_rows=max_result_rows, max_result_bytes=max_result_bytes,
        )
    ...
    async def drop_index(
        ...
        return await _cancelable_call(
            self._session.drop_index, database, collection, name,
            request_id=request_id, timeout_ms=timeout_ms, cancellation=cancellation,
            max_result_rows=max_result_rows, max_result_bytes=max_result_bytes,
        )
    ...
    async def get_more(
        ...
        *,
        ...
        return await _cancelable_call(
            ...
            batch_size=batch_size, plan_diagnostics=plan_diagnostics,
            execution_stats=execution_stats,
    ...
    async def find_one_and_replace(
        ...
        *,
        ...
        return await _cancelable_call(
            self._session.find_one_and_replace,
            database, collection, filter, replacement,
            projection=projection, sort=sort, return_document=return_document,
    ...
    async def find_one_and_update(
        self, database: str, collection: str, filter: Any, update: Any, *,
        projection: Any = None, sort: Any = None, return_document: bool = False,
        upsert: bool = False, request_id: Optional[UUID] = None,
        timeout_ms: Optional[int] = None, cancellation: Optional[CancellationToken] = None,
        max_result_rows: Optional[int] = None, max_result_bytes: Optional[int] = None,
    ) -> dict[str, Any]:
        return await _cancelable_call(
            self._session.find_one_and_update, database, collection, filter, update,
            projection=projection, sort=sort, return_document=return_document,
            upsert=upsert, request_id=request_id, timeout_ms=timeout_ms,
            cancellation=cancellation, max_result_rows=max_result_rows,
            max_result_bytes=max_result_bytes,
    ...
    async def update_one(
        ...
        return await _cancelable_call(
            self._session.update_one, database, collection, filter, update,
            upsert=upsert, request_id=request_id, timeout_ms=timeout_ms,
            cancellation=cancellation, max_result_rows=max_result_rows,
            max_result_bytes=max_result_bytes,
        )
    ...
    async def update_many(
        ...
        return await _cancelable_call(
            self._session.update_many, database, collection, filter, update,
            upsert=upsert, request_id=request_id, timeout_ms=timeout_ms,
            cancellation=cancellation, max_result_rows=max_result_rows,
            max_result_bytes=max_result_bytes,
        )
