...
const V16_SCHEMA_VERSION: u32 = 16;
const V17_SCHEMA_VERSION: u32 = 17;
...
const V8_MANIFEST_DIGEST_VERSION: u32 = 8;
const V9_MANIFEST_DIGEST_VERSION: u32 = 9;
...
const V8_MANIFEST_DIGEST_DOMAIN: &[u8] = b"briskdb.manifest.semantic-root.v8\0";
const V9_MANIFEST_DIGEST_DOMAIN: &[u8] = b"briskdb.manifest.semantic-root.v9\0";
...
const V16_DOWNGRADE_FENCE_SQL: &str = "CREATE TABLE briskdb_metadata (
    requires_manifest_version INTEGER NOT NULL
        CHECK (requires_manifest_version >= 16)
) STRICT";
const V17_DOWNGRADE_FENCE_SQL: &str = "CREATE TABLE briskdb_metadata (
    requires_manifest_version INTEGER NOT NULL
        CHECK (requires_manifest_version >= 17)
) STRICT";
const V18_DOWNGRADE_FENCE_SQL: &str = "CREATE TABLE briskdb_metadata (
    requires_manifest_version INTEGER NOT NULL
        CHECK (requires_manifest_version >= 18)
) STRICT";
const V19_DOWNGRADE_FENCE_SQL: &str = "CREATE TABLE briskdb_metadata (
    requires_manifest_version INTEGER NOT NULL
        CHECK (requires_manifest_version >= 19)
) STRICT";
...
const V16_DOCUMENT_IDENTITIES_TABLE_SQL: &str = "CREATE TABLE briskdb_document_identities (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    database_high_water INTEGER NOT NULL CHECK (database_high_water >= 0),
    collection_high_water INTEGER NOT NULL CHECK (collection_high_water >= 0)
) STRICT";
const V17_DOCUMENT_INDEX_IDENTITIES_TABLE_SQL: &str =
    "CREATE TABLE briskdb_document_index_identities (
    index_id INTEGER PRIMARY KEY CHECK (index_id > 0),
    collection_id INTEGER NOT NULL,
    index_name TEXT NOT NULL COLLATE BINARY,
    UNIQUE (collection_id, index_name),
    FOREIGN KEY (collection_id, index_name)
        REFERENCES briskdb_document_indexes (collection_id, index_name)
        ON DELETE CASCADE
) STRICT";
const V17_DOCUMENT_INDEX_ALLOCATOR_TABLE_SQL: &str =
    "CREATE TABLE briskdb_document_index_allocator (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    index_high_water INTEGER NOT NULL CHECK (index_high_water >= 0)
) STRICT";
const V18_DOCUMENT_INDEX_STORAGE_TABLE_SQL: &str = "CREATE TABLE briskdb_document_index_storage (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
...
const V19_DOCUMENT_INDEX_OPERATION_TABLE_SQL: &str =
...
const V16_DOCUMENT_DELETION_TABLE_SQL: &str = "CREATE TABLE briskdb_document_deletion (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    database_id INTEGER NOT NULL CHECK (database_id > 0),
    ...
    next_shard INTEGER NOT NULL CHECK (next_shard BETWEEN 0 AND shard_count),
    FOREIGN KEY (database_id)
        REFERENCES briskdb_document_databases (database_id) ON DELETE RESTRICT,
    FOREIGN KEY (collection_id)
        REFERENCES briskdb_document_collections (collection_id) ON DELETE RESTRICT
) STRICT";
...
const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        from: V15_SCHEMA_VERSION,
        to: V16_SCHEMA_VERSION,
        name: "durable_document_identities_and_deletion",
        apply: migrate_v15_to_v16,
        validate: validate_v16,
    },
    Migration {
        from: V16_SCHEMA_VERSION,
        to: V17_SCHEMA_VERSION,
        name: "durable_document_index_identities",
        apply: migrate_v16_to_v17,
        validate: validate_v17,
    },
    Migration {
        from: V17_SCHEMA_VERSION,
        to: V18_SCHEMA_VERSION,
        name: "document_index_storage_and_upgrade_journal",
        apply: migrate_v17_to_v18,
        validate: validate_v18,
    },
    Migration {
        from: V18_SCHEMA_VERSION,
        to: V19_SCHEMA_VERSION,
        name: "document_index_build_and_maintenance_authority",
        apply: migrate_v18_to_v19,
        validate: validate_v19,
    },
    Migration {
        from: V19_SCHEMA_VERSION,
        to: V20_SCHEMA_VERSION,
        name: "document_secondary_uniqueness_writer_fence",
    ...
    let document_catalog_steady = connection
        .query_row(
            "SELECT NOT EXISTS (SELECT 1 FROM briskdb_document_provisioning)
                AND NOT EXISTS (SELECT 1 FROM briskdb_document_deletion)
                AND NOT EXISTS (SELECT 1 FROM briskdb_document_index_storage WHERE lifecycle_state = 2)
                AND NOT EXISTS (SELECT 1 FROM briskdb_document_index_operation)",
            [],
            |row| row.get::<_, i64>(0),
        )
        .map_err(|error| {
    ...
    connection
        .execute_batch(
            "DROP TABLE IF EXISTS briskdb_document_index_operation;
             ...
             DROP TABLE IF EXISTS briskdb_document_deletion;
             DROP TABLE IF EXISTS briskdb_document_identities;
...
fn downgrade_v15_manifest_to_v14_for_test(
    ...
    if read_identity(connection)?.1 >= i64::from(V16_SCHEMA_VERSION) {
        downgrade_v16_manifest_to_v15_for_test(connection, shard_count)?;
    }
    connection
...
#[cfg(test)]
fn downgrade_v16_manifest_to_v15_for_test(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    if read_identity(connection)?.1 >= i64::from(V17_SCHEMA_VERSION) {
        downgrade_v17_manifest_to_v16_for_test(connection, shard_count)?;
    }
    connection
        .execute_batch(
    ...
    connection
        ...
        .map_err(sqlite_error::storage)?;
    set_identity(connection, V15_SCHEMA_VERSION)?;
    refresh_manifest_digest(connection)?;
    validate_v15(connection, shard_count, &schema_objects(connection)?)?;
    Ok(())
}

#[cfg(test)]
fn downgrade_v17_manifest_to_v16_for_test(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    if read_identity(connection)?.1 >= i64::from(V18_SCHEMA_VERSION) {
        downgrade_v18_manifest_to_v17_for_test(connection, shard_count)?;
    }
    connection
        .execute_batch(
        ...
        .map_err(sqlite_error::storage)?;
    connection
        .execute_batch(V16_DOWNGRADE_FENCE_SQL)
        .map_err(sqlite_error::storage)?;
    connection
        .execute(
            "INSERT INTO briskdb_metadata VALUES (?1)",
    ...
    connection
        .execute(
            "UPDATE briskdb_integrity SET manifest_digest_version = ?1 WHERE singleton = 1",
            [V8_MANIFEST_DIGEST_VERSION],
        )
        .map_err(sqlite_error::storage)?;
    set_identity(connection, V16_SCHEMA_VERSION)?;
    refresh_manifest_digest(connection)?;
    validate_v16(connection, shard_count, &schema_objects(connection)?)?;
    Ok(())
}

#[cfg(test)]
pub(super) fn downgrade_v18_manifest_to_v17_for_test(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    if read_identity(connection)?.1 >= i64::from(V19_SCHEMA_VERSION) {
        downgrade_v19_manifest_to_v18_for_test(connection, shard_count)?;
    }
    connection
        .execute_batch("DROP TABLE briskdb_document_index_storage; DROP TABLE briskdb_metadata;")
        .map_err(sqlite_error::storage)?;
    connection
        .execute_batch(V17_DOWNGRADE_FENCE_SQL)
        .map_err(sqlite_error::storage)?;
    connection
        .execute(
            "INSERT INTO briskdb_metadata VALUES (?1)",
    ...
    connection
        ...
        .map_err(sqlite_error::storage)?;
    set_identity(connection, V17_SCHEMA_VERSION)?;
    refresh_manifest_digest(connection)?;
    validate_v17(connection, shard_count, &schema_objects(connection)?)?;
    Ok(())
}

#[cfg(test)]
pub(super) fn downgrade_v19_manifest_to_v18_for_test(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    ...
    connection
        .execute_batch("DROP TABLE briskdb_document_index_operation; DROP TABLE briskdb_metadata;")
        .map_err(sqlite_error::storage)?;
    connection
        .execute_batch(V18_DOWNGRADE_FENCE_SQL)
        .map_err(sqlite_error::storage)?;
    connection
        .execute(
            "INSERT INTO briskdb_metadata VALUES (?1)",
    ...
    connection
        ...
        .map_err(sqlite_error::storage)?;
    set_identity(connection, V18_SCHEMA_VERSION)?;
    refresh_manifest_digest(connection)?;
    validate_v18(connection, shard_count, &schema_objects(connection)?)?;
    Ok(())
}

#[cfg(test)]
fn downgrade_v20_manifest_to_v19_for_test(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    connection
        ...
        .map_err(sqlite_error::storage)?;
    connection
        .execute_batch(V19_DOWNGRADE_FENCE_SQL)
        .map_err(sqlite_error::storage)?;
    connection
        .execute(
            "INSERT INTO briskdb_metadata VALUES (?1)",
...
fn create_v16_schema(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
    create_v15_schema(transaction, shard_count)?;
    migrate_v15_to_v16(transaction, shard_count)
}

fn create_v17_schema(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
    create_v16_schema(transaction, shard_count)?;
    migrate_v16_to_v17(transaction, shard_count)
}

fn create_v18_schema(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
    create_v17_schema(transaction, shard_count)?;
    migrate_v17_to_v18(transaction, shard_count)
}

fn create_v19_schema(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
    create_v18_schema(transaction, shard_count)?;
    migrate_v18_to_v19(transaction, shard_count)
}

fn create_v20_schema(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
...
#[cfg(test)]
fn migrate_interrupted_legacy_to_v16(
    transaction: &Transaction<'_>,
    shard_count: u16,
) -> EngineResult<()> {
    transaction
...
#[cfg(test)]
fn migrate_interrupted_legacy_to_v17(
    transaction: &Transaction<'_>,
    shard_count: u16,
) -> EngineResult<()> {
    transaction
...
#[cfg(test)]
fn migrate_interrupted_legacy_to_v18(
    transaction: &Transaction<'_>,
    shard_count: u16,
) -> EngineResult<()> {
    transaction
...
#[cfg(test)]
fn migrate_interrupted_legacy_to_v19(
    transaction: &Transaction<'_>,
    shard_count: u16,
) -> EngineResult<()> {
    transaction
...
fn migrate_v15_to_v16(transaction: &Transaction<'_>, _shard_count: u16) -> EngineResult<()> {
    transaction
        .execute_batch(V16_DOCUMENT_IDENTITIES_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    transaction
        .execute_batch(V16_DOCUMENT_DELETION_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    transaction
        .execute_batch(
            "INSERT INTO briskdb_document_identities
             (singleton, database_high_water, collection_high_water)
         SELECT 1, (SELECT COALESCE(MAX(database_id), 0) FROM briskdb_document_databases),
                   (SELECT COALESCE(MAX(collection_id), 0) FROM briskdb_document_collections);
         DROP TABLE briskdb_metadata;",
        )
        .map_err(sqlite_error::storage)?;
    transaction
...
fn migrate_v16_to_v17(transaction: &Transaction<'_>, _shard_count: u16) -> EngineResult<()> {
    // The validated v16 catalog is bounded. Assign IDs deterministically without
    // decoding or rewriting a single byte of existing index specifications.
    transaction
        .execute_batch(V17_DOCUMENT_INDEX_IDENTITIES_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    transaction
        .execute_batch(V17_DOCUMENT_INDEX_ALLOCATOR_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    transaction
        .execute_batch(
            "INSERT INTO briskdb_document_index_identities (index_id, collection_id, index_name)
             SELECT row_number() OVER (ORDER BY collection_id, index_name COLLATE BINARY),
                    collection_id, index_name FROM briskdb_document_indexes;
             INSERT INTO briskdb_document_index_allocator (singleton, index_high_water)
             SELECT 1, COALESCE(MAX(index_id), 0) FROM briskdb_document_index_identities;
             DROP TABLE briskdb_metadata;",
        )
        .map_err(sqlite_error::storage)?;
    transaction
...
fn migrate_v17_to_v18(transaction: &Transaction<'_>, shard_count: u16) -> EngineResult<()> {
    transaction
        .execute_batch(V18_DOCUMENT_INDEX_STORAGE_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    // Only empty derived storage is provisioned by this upgrade. Existing
    // document and declaration bytes, IDs, and index lifecycles are untouched.
    transaction.execute(
        "INSERT INTO briskdb_document_index_storage
         SELECT 1, 1, CASE WHEN EXISTS (SELECT 1 FROM briskdb_document_collections) THEN 2 ELSE 1 END,
                ?1, CASE WHEN EXISTS (SELECT 1 FROM briskdb_document_collections) THEN 0 ELSE ?1 END",
        [shard_count],
    ).map_err(sqlite_error::storage)?;
    transaction
        .execute_batch("DROP TABLE briskdb_metadata;")
...
fn migrate_v18_to_v19(transaction: &Transaction<'_>, _shard_count: u16) -> EngineResult<()> {
    transaction
        .execute_batch(V19_DOCUMENT_INDEX_OPERATION_TABLE_SQL)
        .map_err(sqlite_error::storage)?;
    transaction
        .execute_batch("DROP TABLE briskdb_metadata")
        .map_err(sqlite_error::storage)?;
...
fn migrate_v19_to_v20(transaction: &Transaction<'_>, _shard_count: u16) -> EngineResult<()> {
    // Physical entries are unchanged. Only v20 writers understand collection
    // fencing and cross-shard secondary uniqueness; earlier readers must stop
    // before treating a Ready unique declaration as ordinary derived storage.
    transaction
        .execute_batch("DROP TABLE briskdb_metadata")
        .map_err(sqlite_error::storage)?;
    transaction
...
fn v16_objects() -> Vec<SchemaObject> {
    let mut objects = v15_objects();
    for name in ["briskdb_document_identities", "briskdb_document_deletion"] {
        objects.push(SchemaObject {
            object_type: "table".to_owned(),
            name: name.to_owned(),
        });
...
fn v17_objects() -> Vec<SchemaObject> {
    let mut objects = v16_objects();
    for name in [
        "briskdb_document_index_identities",
        "briskdb_document_index_allocator",
    ] {
        objects.push(SchemaObject {
            object_type: "table".to_owned(),
            name: name.to_owned(),
...
fn v18_objects() -> Vec<SchemaObject> {
    let mut objects = v17_objects();
    objects.push(SchemaObject {
        object_type: "table".into(),
        name: "briskdb_document_index_storage".into(),
    });
    objects.sort_by(|left, right| {
        (&left.object_type, &left.name).cmp(&(&right.object_type, &right.name))
    });
...
fn v19_objects() -> Vec<SchemaObject> {
    let mut objects = v18_objects();
    objects.push(SchemaObject {
        object_type: "table".into(),
        name: "briskdb_document_index_operation".into(),
    });
    objects.sort_by(|left, right| {
        (&left.object_type, &left.name).cmp(&(&right.object_type, &right.name))
    });
...
fn validate_table(
    ...
    let pragma = match table {
        ...
        "briskdb_document_identities" => {
            "SELECT cid, name, type, \"notnull\", dflt_value, pk, hidden
             FROM pragma_table_xinfo('briskdb_document_identities') LIMIT ?1"
        }
        "briskdb_document_deletion" => {
            "SELECT cid, name, type, \"notnull\", dflt_value, pk, hidden
             FROM pragma_table_xinfo('briskdb_document_deletion') LIMIT ?1"
        }
        "briskdb_document_index_identities" => {
            "SELECT cid, name, type, \"notnull\", dflt_value, pk, hidden
             FROM pragma_table_xinfo('briskdb_document_index_identities') LIMIT ?1"
        }
        "briskdb_document_index_allocator" => {
            "SELECT cid, name, type, \"notnull\", dflt_value, pk, hidden
             FROM pragma_table_xinfo('briskdb_document_index_allocator') LIMIT ?1"
        }
        "briskdb_document_index_storage" => {
            "SELECT cid, name, type, \"notnull\", dflt_value, pk, hidden
             FROM pragma_table_xinfo('briskdb_document_index_storage') LIMIT ?1"
        }
...
fn validate_v16(
    ...
    let mut snapshot = validate_v12_features(
        ...
        IntegrityManifestDefinition {
            ...
            downgrade_fence_sql: V16_DOWNGRADE_FENCE_SQL,
            expected_objects: &v16_objects(),
            expected_manifest_digest_version: V8_MANIFEST_DIGEST_VERSION,
            generated_ids: true,
        },
    )?;
    let catalog = snapshot.logical_catalog.take().ok_or_else(|| {
        EngineError::new(
            EngineErrorKind::Internal,
            "document lifecycle validation omitted the logical table catalog",
        )
    })?;
    let indexes = validate_global_indexes(connection, &catalog)?;
    snapshot.logical_catalog = Some(catalog.with_global_indexes(indexes));
...
fn validate_v17(
    ...
    let mut snapshot = validate_v12_features(
        ...
        IntegrityManifestDefinition {
            ...
            downgrade_fence_sql: V17_DOWNGRADE_FENCE_SQL,
            expected_objects: &v17_objects(),
            expected_manifest_digest_version: V9_MANIFEST_DIGEST_VERSION,
            generated_ids: true,
        },
    )?;
    let catalog = snapshot.logical_catalog.take().ok_or_else(|| {
        EngineError::new(
            EngineErrorKind::Internal,
            "document index validation omitted the logical table catalog",
        )
    })?;
    let indexes = validate_global_indexes(connection, &catalog)?;
    snapshot.logical_catalog = Some(catalog.with_global_indexes(indexes));
...
fn validate_v18(
    ...
    let mut snapshot = validate_v12_features(
        ...
        IntegrityManifestDefinition {
            ...
            downgrade_fence_sql: V18_DOWNGRADE_FENCE_SQL,
            expected_objects: &v18_objects(),
            expected_manifest_digest_version: V10_MANIFEST_DIGEST_VERSION,
            generated_ids: true,
        },
    )?;
    let catalog = snapshot.logical_catalog.take().ok_or_else(|| {
        EngineError::new(
            EngineErrorKind::Internal,
            "document index storage validation omitted the logical table catalog",
        )
    })?;
    let indexes = validate_global_indexes(connection, &catalog)?;
    snapshot.logical_catalog = Some(catalog.with_global_indexes(indexes));
...
fn validate_v19(
    ...
    let mut snapshot = validate_v12_features(
        ...
        objects,
        IntegrityManifestDefinition {
            version: V19_SCHEMA_VERSION,
            downgrade_fence_sql: V19_DOWNGRADE_FENCE_SQL,
            expected_objects: &v19_objects(),
            expected_manifest_digest_version: V11_MANIFEST_DIGEST_VERSION,
            generated_ids: true,
        },
    )?;
    let catalog = snapshot.logical_catalog.take().ok_or_else(|| {
...
fn validate_v20(
    ...
    let mut snapshot = validate_v12_features(
        ...
        objects,
        IntegrityManifestDefinition {
            version: V20_SCHEMA_VERSION,
            downgrade_fence_sql: V20_DOWNGRADE_FENCE_SQL,
            expected_objects: &v19_objects(),
            expected_manifest_digest_version: V12_MANIFEST_DIGEST_VERSION,
            generated_ids: true,
        },
    )?;
    let catalog = snapshot.logical_catalog.take().ok_or_else(|| {
        EngineError::new(
            EngineErrorKind::Internal,
            "document unique-index validation omitted the logical catalog",
        )
    })?;
    let indexes = validate_global_indexes(connection, &catalog)?;
    snapshot.logical_catalog = Some(catalog.with_global_indexes(indexes));
...
pub(super) fn validate_document_catalog(
    ...
    if read_identity(connection)?.1 >= i64::from(V16_SCHEMA_VERSION) {
        validate_document_lifecycle(connection, shard_count)?;
    }
    if read_identity(connection)?.1 >= i64::from(V17_SCHEMA_VERSION) {
        validate_document_index_identities(connection)?;
    }
    if read_identity(connection)?.1 >= i64::from(V18_SCHEMA_VERSION) {
        validate_document_index_storage(connection, shard_count)?;
    }
    if read_identity(connection)?.1 >= i64::from(V19_SCHEMA_VERSION) {
        validate_document_index_operation(connection, shard_count)?;
    }
    validate_foreign_keys(connection)?;
    Ok(())
}

fn validate_document_index_operation(
    connection: &Connection,
    shard_count: u16,
) -> EngineResult<()> {
    validate_table(
        ...
        &[
            TableColumn::expected(0, "singleton", "INTEGER", false, 1),
            TableColumn::expected(1, "index_id", "INTEGER", true, 0),
            TableColumn::expected(2, "operation_kind", "INTEGER", true, 0),
            TableColumn::expected(3, "operation_id", "BLOB", true, 0),
            TableColumn::expected(4, "shard_count", "INTEGER", true, 0),
            TableColumn::expected(5, "next_shard", "INTEGER", true, 0),
        ],
        true,
    )?;
    validate_table_sql(
        connection,
        "briskdb_document_index_operation",
        V19_DOCUMENT_INDEX_OPERATION_TABLE_SQL,
    )?;
    let invalid: bool = connection.query_row(
        "SELECT (SELECT count(*) FROM briskdb_document_index_operation) > 1
         OR EXISTS (SELECT 1 FROM briskdb_document_index_operation AS o
             LEFT JOIN briskdb_document_index_identities AS d ON d.index_id = o.index_id
             LEFT JOIN briskdb_document_indexes AS i ON i.collection_id = d.collection_id AND i.index_name = d.index_name
             LEFT JOIN briskdb_document_collections AS c ON c.collection_id = d.collection_id
             WHERE o.singleton <> 1 OR o.index_id <= 0 OR o.operation_kind NOT IN (1, 2, 3)
                OR typeof(o.operation_id) <> 'blob' OR length(o.operation_id) <> 32
                OR o.shard_count <> ?1 OR o.next_shard NOT BETWEEN 0 AND o.shard_count
                OR d.index_id IS NULL OR i.index_name IS NULL OR i.is_builtin <> 0
                OR i.lifecycle_state <> 2 OR c.lifecycle_state <> 2
                OR (?2 = 0 AND o.operation_kind IN (1, 3) AND i.is_unique <> 0)
                OR EXISTS (SELECT 1 FROM briskdb_document_provisioning)
                OR EXISTS (SELECT 1 FROM briskdb_document_deletion)
                OR EXISTS (SELECT 1 FROM briskdb_document_index_storage WHERE lifecycle_state <> 1))
         OR (?2 = 0 AND EXISTS (SELECT 1 FROM briskdb_document_indexes WHERE is_builtin = 0 AND is_unique = 1 AND lifecycle_state = 1))",
        rusqlite::params![shard_count, read_identity(connection)?.1 >= i64::from(V20_SCHEMA_VERSION)], |row| row.get(0),
    ).map_err(|error| manifest_read_error(error, "failed to validate document index operation journal"))?;
    if invalid {
        return Err(invalid_document_catalog(
            "document index operation or physical authority is invalid",
        ));
    }
    Ok(())
}

fn validate_document_index_storage(connection: &Connection, shard_count: u16) -> EngineResult<()> {
    validate_table(
        connection,
        "briskdb_document_index_storage",
        &[
            TableColumn::expected(0, "singleton", "INTEGER", false, 1),
            TableColumn::expected(1, "entry_format_version", "INTEGER", true, 0),
            TableColumn::expected(2, "lifecycle_state", "INTEGER", true, 0),
            TableColumn::expected(3, "shard_count", "INTEGER", true, 0),
            TableColumn::expected(4, "next_shard", "INTEGER", true, 0),
        ],
        true,
    ...
    validate_table_sql(
        connection,
        "briskdb_document_index_storage",
        V18_DOCUMENT_INDEX_STORAGE_TABLE_SQL,
    )?;
    let valid: bool = connection
        .query_row(
            "SELECT (SELECT count(*) FROM briskdb_document_index_storage) = 1
         AND EXISTS (SELECT 1 FROM briskdb_document_index_storage
         WHERE singleton = 1 AND entry_format_version = 1 AND lifecycle_state IN (1, 2)
         AND shard_count = ?1 AND next_shard BETWEEN 0 AND shard_count
         AND (lifecycle_state = 2 OR next_shard = shard_count))",
            [shard_count],
            |row| row.get(0),
        )
        .map_err(|error| {
            manifest_read_error(error, "failed to validate document index storage journal")
        })?;
    if !valid {
        return Err(EngineError::new(
            EngineErrorKind::DataCorruption,
            "document index storage journal is invalid",
        ));
    }
    Ok(())
}

fn validate_document_index_identities(connection: &Connection) -> EngineResult<()> {
    validate_table(
        connection,
        "briskdb_document_index_identities",
        &[
    ...
    validate_table_sql(
        connection,
        "briskdb_document_index_identities",
        V17_DOCUMENT_INDEX_IDENTITIES_TABLE_SQL,
    )?;
    validate_table(
        connection,
        "briskdb_document_index_allocator",
        &[
            TableColumn::expected(0, "singleton", "INTEGER", false, 1),
            TableColumn::expected(1, "index_high_water", "INTEGER", true, 0),
        ],
        true,
    )?;
    validate_table_sql(
        connection,
        "briskdb_document_index_allocator",
        V17_DOCUMENT_INDEX_ALLOCATOR_TABLE_SQL,
    )?;
    validate_document_catalog_count(
        connection,
        "briskdb_document_index_identities",
        MAX_DOCUMENT_INDEXES,
        "document index identities",
    )?;
    let valid: bool = connection.query_row(
        "SELECT (SELECT count(*) FROM briskdb_document_index_allocator) = 1
         AND EXISTS (SELECT 1 FROM briskdb_document_index_allocator
             WHERE singleton = 1 AND typeof(index_high_water) = 'integer'
               AND index_high_water >= 0
               AND index_high_water >= (SELECT COALESCE(MAX(index_id), 0) FROM briskdb_document_index_identities))
         AND NOT EXISTS (SELECT 1 FROM briskdb_document_indexes AS i
         ...
         AND NOT EXISTS (SELECT 1 FROM briskdb_document_index_identities AS d
             LEFT JOIN briskdb_document_indexes AS i
             ON d.collection_id = i.collection_id AND d.index_name = i.index_name
             WHERE typeof(d.index_id) <> 'integer' OR d.index_id <= 0
               OR typeof(d.collection_id) <> 'integer' OR typeof(d.index_name) <> 'text'
               OR i.collection_id IS NULL)",
        [], |row| row.get(0),
    ).map_err(|error| manifest_read_error(error, "failed to validate document index identities"))?;
    if !valid {
        return Err(invalid_document_catalog(
            "document index identities or high-water mark are missing or inconsistent",
        ));
    }
    Ok(())
}

fn validate_document_lifecycle(connection: &Connection, shard_count: u16) -> EngineResult<()> {
    validate_table(
        connection,
        "briskdb_document_identities",
        &[
            TableColumn::expected(0, "singleton", "INTEGER", false, 1),
            TableColumn::expected(1, "database_high_water", "INTEGER", true, 0),
            TableColumn::expected(2, "collection_high_water", "INTEGER", true, 0),
        ],
        true,
    )?;
    validate_table_sql(
        connection,
        "briskdb_document_identities",
        V16_DOCUMENT_IDENTITIES_TABLE_SQL,
    )?;
    validate_table(
        connection,
        "briskdb_document_deletion",
        &[
            TableColumn::expected(0, "singleton", "INTEGER", false, 1),
            TableColumn::expected(1, "database_id", "INTEGER", true, 0),
            TableColumn::expected(2, "collection_id", "INTEGER", false, 0),
            TableColumn::expected(3, "operation_id", "BLOB", true, 0),
            TableColumn::expected(4, "shard_count", "INTEGER", true, 0),
            TableColumn::expected(5, "next_shard", "INTEGER", true, 0),
        ],
    ...
    validate_table_sql(
        connection,
        "briskdb_document_deletion",
        V16_DOCUMENT_DELETION_TABLE_SQL,
    )?;
    let valid: bool = connection.query_row(
        "SELECT (SELECT count(*) FROM briskdb_document_identities) = 1
          AND EXISTS (SELECT 1 FROM briskdb_document_identities
             WHERE singleton = 1 AND typeof(database_high_water) = 'integer'
               AND typeof(collection_high_water) = 'integer'
               AND database_high_water >= (SELECT COALESCE(MAX(database_id), 0) FROM briskdb_document_databases)
               AND collection_high_water >= (SELECT COALESCE(MAX(collection_id), 0) FROM briskdb_document_collections))",
        [], |row| row.get(0),
    ).map_err(|error| manifest_read_error(error, "failed to validate document identities"))?;
    if !valid {
        return Err(invalid_document_catalog(
            "document identity high-water marks are missing or inconsistent",
        ));
    }
    let invalid: bool = connection.query_row(
        "SELECT count(*) > 1 OR COALESCE(MAX(
             singleton <> 1 OR typeof(database_id) <> 'integer' OR database_id <= 0
             OR (collection_id IS NOT NULL AND (typeof(collection_id) <> 'integer' OR collection_id <= 0))
             OR typeof(operation_id) <> 'blob' OR length(operation_id) <> 32
             OR typeof(shard_count) <> 'integer' OR shard_count <> ?1
             OR typeof(next_shard) <> 'integer' OR next_shard NOT BETWEEN 0 AND shard_count
             OR EXISTS (SELECT 1 FROM briskdb_document_provisioning)
             OR NOT EXISTS (SELECT 1 FROM briskdb_document_databases AS d WHERE d.database_id = briskdb_document_deletion.database_id)
             OR (collection_id IS NOT NULL AND NOT EXISTS (
                 SELECT 1 FROM briskdb_document_collections AS c
                 WHERE c.collection_id = briskdb_document_deletion.collection_id
                   AND c.database_id = briskdb_document_deletion.database_id AND c.lifecycle_state = 2))
         ), 0) FROM briskdb_document_deletion",
        [shard_count], |row| row.get(0),
    ).map_err(|error| manifest_read_error(error, "failed to validate document deletion journal"))?;
    if invalid {
        return Err(invalid_document_catalog(
            "document deletion journal is inconsistent with its catalog",
        ));
    }
    Ok(())
}
...
const V8_DOCUMENT_IDENTITIES_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_identities",
    columns: &["singleton", "database_high_water", "collection_high_water"],
    sql: "SELECT singleton, database_high_water, collection_high_water FROM briskdb_document_identities ORDER BY singleton",
};
const V8_DOCUMENT_DELETION_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_deletion",
    columns: &[
        "singleton",
        "database_id",
        "collection_id",
    ...
    sql: "SELECT singleton, database_id, collection_id, operation_id, shard_count, next_shard FROM briskdb_document_deletion ORDER BY singleton",
};

const V9_DOCUMENT_INDEX_IDENTITIES_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_index_identities",
    columns: &["index_id", "collection_id", "index_name"],
    sql: "SELECT index_id, collection_id, index_name FROM briskdb_document_index_identities ORDER BY index_id",
};
const V9_DOCUMENT_INDEX_ALLOCATOR_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_index_allocator",
    columns: &["singleton", "index_high_water"],
    sql: "SELECT singleton, index_high_water FROM briskdb_document_index_allocator ORDER BY singleton",
};
const V10_DOCUMENT_INDEX_STORAGE_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_index_storage",
    columns: &[
        "singleton",
    ...
    sql: "SELECT singleton, entry_format_version, lifecycle_state, shard_count, next_shard FROM briskdb_document_index_storage ORDER BY singleton",
};
const V11_DOCUMENT_INDEX_OPERATION_DIGEST_QUERY: ManifestDigestQuery = ManifestDigestQuery {
    table: "briskdb_document_index_operation",
    columns: &[
        "singleton",
        "index_id",
        "operation_kind",
        ...
        | V12_MANIFEST_DIGEST_VERSION => {
            ...
            for query in V1_MANIFEST_DIGEST_QUERIES {
                ...
                if query.table == "briskdb_logical_databases" {
                    ...
                    if digest_version >= V8_MANIFEST_DIGEST_VERSION {
                        queries.push(&V8_DOCUMENT_IDENTITIES_DIGEST_QUERY);
                        queries.push(&V8_DOCUMENT_DELETION_DIGEST_QUERY);
                    }
                    if digest_version >= V9_MANIFEST_DIGEST_VERSION {
                        queries.push(&V9_DOCUMENT_INDEX_IDENTITIES_DIGEST_QUERY);
                        queries.push(&V9_DOCUMENT_INDEX_ALLOCATOR_DIGEST_QUERY);
                    }
                    if digest_version >= V10_MANIFEST_DIGEST_VERSION {
                        queries.push(&V10_DOCUMENT_INDEX_STORAGE_DIGEST_QUERY);
                    }
            ...
            (
                ...
                } else if digest_version == V10_MANIFEST_DIGEST_VERSION {
                    V10_MANIFEST_DIGEST_DOMAIN
                } else if digest_version == V9_MANIFEST_DIGEST_VERSION {
                    V9_MANIFEST_DIGEST_DOMAIN
                } else if digest_version == V8_MANIFEST_DIGEST_VERSION {
                    V8_MANIFEST_DIGEST_DOMAIN
                } else {
                    V7_MANIFEST_DIGEST_DOMAIN
                },
                queries,
            )
        }
...
mod tests {
    ...
    const V15_PLAN: MigrationPlan<'static> = MigrationPlan {
        current_version: V15_SCHEMA_VERSION,
        migrations: MIGRATIONS,
        initialize_current: create_v15_schema,
        initialize_interrupted_legacy: migrate_interrupted_legacy_to_v15,
    };

    const V16_PLAN: MigrationPlan<'static> = MigrationPlan {
        current_version: V16_SCHEMA_VERSION,
        migrations: MIGRATIONS,
        initialize_current: create_v16_schema,
        initialize_interrupted_legacy: migrate_interrupted_legacy_to_v16,
    };

    const V17_PLAN: MigrationPlan<'static> = MigrationPlan {
        current_version: V17_SCHEMA_VERSION,
        migrations: MIGRATIONS,
        initialize_current: create_v17_schema,
        initialize_interrupted_legacy: migrate_interrupted_legacy_to_v17,
    };

    const V18_PLAN: MigrationPlan<'static> = MigrationPlan {
        current_version: V18_SCHEMA_VERSION,
        migrations: MIGRATIONS,
        initialize_current: create_v18_schema,
        initialize_interrupted_legacy: migrate_interrupted_legacy_to_v18,
    };

    const V19_PLAN: MigrationPlan<'static> = MigrationPlan {
        current_version: V19_SCHEMA_VERSION,
        migrations: MIGRATIONS,
        initialize_current: create_v19_schema,
    ...
    fn v20_unique_authority_upgrade_is_atomic_and_fences_v19_readers() {
        ...
        .flat_map(|phase| [false, true].map(|panic| (phase, panic)))
        {
            ...
            downgrade_v20_manifest_to_v19_for_test(&connection, 4).unwrap();
            let before = stored_manifest_digest(&connection);
            let attempt = catch_unwind(AssertUnwindSafe(|| {
                load_or_create_with_hook(&mut connection, 4, |point| {
                    if point.from == V19_SCHEMA_VERSION && point.phase == phase {
                        if inject_panic {
                            panic!("injected unique writer-fence upgrade panic");
                        }
                        return Err(EngineError::new(
                            EngineErrorKind::Internal,
                            "injected unique writer-fence upgrade error",
                        ));
                    }
                    Ok(())
                })
            ...
            assert_eq!(identity(&connection).1, 19);
            ...
            assert_eq!(
                inspect_with_plan(&connection, 4, V19_PLAN)
                    .unwrap_err()
                    .kind(),
                EngineErrorKind::FailedPrecondition
            );
            assert_eq!(
                ...
                after,
                "old readers must not rewrite the root"
            );
            downgrade_v20_manifest_to_v19_for_test(&connection, 4).unwrap();
            assert_eq!(stored_manifest_digest(&connection), before);
        }
    ...
    #[test]
    fn v19_document_index_authority_upgrade_is_atomic_and_fences_v18_readers() {
        for (phase, inject_panic) in [
            MigrationPhase::AfterSchemaChange,
            MigrationPhase::AfterVersionStamp,
        ]
        ...
        .flat_map(|phase| [false, true].map(|panic| (phase, panic)))
        {
            ...
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
            downgrade_v19_manifest_to_v18_for_test(&connection, 4).unwrap();
            let before = stored_manifest_digest(&connection);
            let attempt = catch_unwind(AssertUnwindSafe(|| {
                load_or_create_with_hook(&mut connection, 4, |point| {
                    if point.from == V18_SCHEMA_VERSION && point.phase == phase {
                        if inject_panic {
                            panic!("injected document index authority migration panic");
                        }
                        return Err(EngineError::new(
                            EngineErrorKind::Internal,
                            "injected document index authority migration error",
                        ));
                    }
                    Ok(())
                })
            ...
            assert_eq!(identity(&connection).1, 18);
            assert_eq!(schema_objects(&connection).unwrap(), v18_objects());
            assert_eq!(stored_manifest_digest(&connection), before);
            load_or_create_manifest(&mut connection, 4).unwrap();
            ...
            assert_eq!(
                connection
                    .query_row(
                        "SELECT count(*) FROM briskdb_document_index_operation",
                        [],
                        |r| r.get::<_, i64>(0)
                    )
                    .unwrap(),
            ...
            assert_eq!(
                inspect_with_plan(&connection, 4, V18_PLAN)
                    .unwrap_err()
                    .kind(),
                EngineErrorKind::FailedPrecondition
            );
    ...
    fn install_document_index_operation_fixture(connection: &Connection) {
        insert_active_document_catalog(connection);
        connection.execute_batch("INSERT INTO briskdb_document_indexes VALUES (11, 'pending', x'0500000000', 0, 0, 1, 2);
            INSERT INTO briskdb_document_index_identities VALUES (2, 11, 'pending');
            UPDATE briskdb_document_index_allocator SET index_high_water = 2;
            INSERT INTO briskdb_document_index_operation VALUES (1, 2, 1, zeroblob(32), 4, 0)").unwrap();
        refresh_manifest_digest(connection).unwrap();
        inspect_with_plan(connection, 4, CURRENT_PLAN).unwrap();
    }

    #[test]
    fn document_index_operation_journal_is_checksummed_scoped_and_exclusive() {
        for mutation in [
            "UPDATE briskdb_document_index_operation SET singleton = 2",
            "UPDATE briskdb_document_index_operation SET operation_kind = 4",
            "UPDATE briskdb_document_index_operation SET operation_id = zeroblob(31)",
            "UPDATE briskdb_document_index_operation SET index_id = 1",
            "UPDATE briskdb_document_index_operation SET index_id = 3",
            "UPDATE briskdb_document_index_operation SET next_shard = 5",
            "UPDATE briskdb_document_index_operation SET next_shard = -1",
            "UPDATE briskdb_document_index_operation SET shard_count = 3",
            "UPDATE briskdb_document_indexes SET lifecycle_state = 1 WHERE index_name = 'pending'",
            "UPDATE briskdb_document_index_storage SET lifecycle_state = 2, next_shard = 0",
            "INSERT INTO briskdb_document_deletion VALUES (1, 7, 11, zeroblob(32), 4, 0)",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
            install_document_index_operation_fixture(&connection);
            connection
        ...
        connection
            .execute_batch(
                "UPDATE briskdb_document_index_operation SET operation_kind = 3, next_shard = 1",
            )
            .unwrap();
        assert_document_manifest_corruption(&connection);
        refresh_manifest_digest(&connection).unwrap();
    ...
    #[test]
    fn only_v20_authorizes_unique_builds_and_ready_secondary_indexes() {
        for old in [false, true] {
            ...
            connection.execute_batch("UPDATE briskdb_document_indexes SET is_unique = 1 WHERE index_name = 'pending'").unwrap();
            refresh_manifest_digest(&connection).unwrap();
            if old {
                assert_document_manifest_corruption(&connection);
            } else {
            ...
            connection.execute_batch("DELETE FROM briskdb_document_index_operation; UPDATE briskdb_document_indexes SET lifecycle_state = 1 WHERE index_name = 'pending'").unwrap();
            refresh_manifest_digest(&connection).unwrap();
            if old {
                assert_document_manifest_corruption(&connection);
            } else {
    ...
    #[test]
    fn v18_index_storage_upgrade_is_atomic_preserves_metadata_and_fences_old_readers() {
        for populated in [false, true] {
            ...
            .flat_map(|phase| [false, true].map(|panic| (phase, panic)))
            {
                ...
                downgrade_v18_manifest_to_v17_for_test(&connection, 4).unwrap();
                let root = stored_manifest_digest(&connection);
                let identities = index_identity_rows(&connection);
                let catalog_digest = |connection: &Connection| {
                    manifest_semantic_digest_for_version(connection, V9_MANIFEST_DIGEST_VERSION)
                };
                let before = catalog_digest(&connection).unwrap();
                let attempt = catch_unwind(AssertUnwindSafe(|| {
                    load_or_create_with_hook(&mut connection, 4, |point| {
                        if point.from == V17_SCHEMA_VERSION && point.phase == phase {
                            if inject_panic {
                                panic!("injected index storage migration panic");
                            }
                            return Err(EngineError::new(
                                EngineErrorKind::Internal,
                                "injected index storage migration failure",
                            ));
                        }
                        Ok(())
                    })
                ...
                assert_eq!(identity(&connection).1, 17);
                assert_eq!(schema_objects(&connection).unwrap(), v17_objects());
                ...
                assert_eq!(
                    ...
                    (
                        ...
                        4,
                        if populated { 0 } else { 4 }
                    )
                );
                assert_eq!(
                    startup_requires_exclusive_ownership(&connection, 4).unwrap(),
                    populated
                );
                let root = stored_manifest_digest(&connection);
                assert_eq!(
                    inspect_with_plan(&connection, 4, V17_PLAN)
                        .unwrap_err()
                        .kind(),
                    EngineErrorKind::FailedPrecondition
                );
                load_or_create_manifest(&mut connection, 4).unwrap();
                assert_eq!(stored_manifest_digest(&connection), root);
                // Reconstruct only the old manifest in this in-memory fixture:
                // every old semantic input must retain its exact digest.
                downgrade_v18_manifest_to_v17_for_test(&connection, 4).unwrap();
                assert_eq!(catalog_digest(&connection).unwrap(), before);
            }
        }
    ...
    #[test]
    fn document_index_storage_journal_is_validated_and_checksum_authority() {
        for mutation in [
            "DELETE FROM briskdb_document_index_storage",
            "UPDATE briskdb_document_index_storage SET singleton = 2",
            "UPDATE briskdb_document_index_storage SET entry_format_version = 2",
            "UPDATE briskdb_document_index_storage SET lifecycle_state = 3",
            "UPDATE briskdb_document_index_storage SET next_shard = -1",
            "UPDATE briskdb_document_index_storage SET next_shard = 5",
            "UPDATE briskdb_document_index_storage SET next_shard = 0",
            "UPDATE briskdb_document_index_storage SET shard_count = 3, next_shard = 3",
            "INSERT INTO briskdb_document_index_storage VALUES (2, 1, 1, 4, 4)",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
            connection
        ...
        connection
            .execute_batch(
                "UPDATE briskdb_document_index_storage SET lifecycle_state = 2, next_shard = 1",
            )
            .unwrap();
        assert_document_manifest_corruption(&connection);
        refresh_manifest_digest(&connection).unwrap();
    ...
    fn index_identity_rows(connection: &Connection) -> Vec<(i64, i64, String)> {
        connection.prepare("SELECT index_id, collection_id, index_name FROM briskdb_document_index_identities ORDER BY index_id")
            .unwrap().query_map([], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)))
            .unwrap().collect::<Result<Vec<_>, _>>().unwrap()
    }

    #[test]
    fn v17_index_identity_upgrade_is_atomic_preserves_declarations_and_fences_old_readers() {
        for (phase, inject_panic) in [
            MigrationPhase::AfterSchemaChange,
            MigrationPhase::AfterVersionStamp,
        ]
        ...
        .flat_map(|phase| [false, true].map(|panic| (phase, panic)))
        {
            ...
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
            downgrade_v17_manifest_to_v16_for_test(&connection, 4).unwrap();
            // Include opaque legacy bytes: this migration must never decode,
            // normalize or reinterpret a retained pending declaration.
            connection
                .execute_batch(
                    "INSERT INTO briskdb_document_collections VALUES
                (5, 7, 'earlier_id', x'0500000000', 1, 1, 1, 1, 1, 2);
                INSERT INTO briskdb_document_indexes VALUES
                (5, '_id_', x'0500000000', 1, 1, 1, 1),
                (11, 'z', x'0500000000', 1, 0, 1, 2),
                (11, 'A', x'0500000001', 0, 0, 1, 2);",
                )
                .unwrap();
            refresh_manifest_digest(&connection).unwrap();
            type StoredIndexRow = (i64, String, Vec<u8>, i64, i64, i64, i64);
            let declarations = |connection: &Connection| -> Vec<StoredIndexRow> {
                connection.prepare("SELECT * FROM briskdb_document_indexes ORDER BY collection_id, index_name COLLATE BINARY")
                    .unwrap().query_map([], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?, r.get(6)?)))
                    .unwrap().collect::<Result<Vec<_>, _>>().unwrap()
            };
            let before = declarations(&connection);
            let root = stored_manifest_digest(&connection);
            let attempt = catch_unwind(AssertUnwindSafe(|| {
                load_or_create_with_hook(&mut connection, 4, |point| {
                    if point.from == V16_SCHEMA_VERSION && point.phase == phase {
                        if inject_panic {
                            panic!("injected index identity migration panic");
                        }
                        return Err(EngineError::new(
                            EngineErrorKind::Internal,
                            "injected index identity migration failure",
                        ));
                    }
                    Ok(())
                })
            ...
            assert_eq!(identity(&connection).1, 16);
            assert_eq!(schema_objects(&connection).unwrap(), v16_objects());
            assert_eq!(stored_manifest_digest(&connection), root);
            assert_eq!(declarations(&connection), before);
            load_or_create_manifest(&mut connection, 4).unwrap();
            assert_eq!(declarations(&connection), before);
            assert_eq!(
                index_identity_rows(&connection),
                [
                    (1, 5, "_id_".into()),
                    (2, 11, "A".into()),
                    (3, 11, "_id_".into()),
            ...
            assert_eq!(stored_manifest_digest(&connection), root);
            assert_eq!(
                inspect_with_plan(&connection, 4, V16_PLAN)
                    .unwrap_err()
                    .kind(),
                EngineErrorKind::FailedPrecondition
            );
    ...
    #[test]
    fn document_index_identities_are_validated_and_checksum_authority() {
        for mutation in [
            "DELETE FROM briskdb_document_index_identities",
            "DELETE FROM briskdb_document_index_allocator",
            "UPDATE briskdb_document_index_allocator SET index_high_water = 0",
            "UPDATE briskdb_document_index_allocator SET index_high_water = -1",
            "UPDATE briskdb_document_index_identities SET index_id = 0",
            "UPDATE briskdb_document_index_identities SET collection_id = 12",
            "UPDATE briskdb_document_index_identities SET index_name = 'absent'",
            "INSERT INTO briskdb_document_index_identities VALUES (2, 11, 'orphan')",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
        ...
        for mutation in [
            "UPDATE briskdb_document_index_allocator SET index_high_water = 100",
            "UPDATE briskdb_document_index_identities SET index_id = 2;
             UPDATE briskdb_document_index_allocator SET index_high_water = 2",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
    ...
    #[test]
    fn document_index_identity_constraints_prevent_aliases_and_cascade_only_mappings() {
        let mut connection = Connection::open_in_memory().unwrap();
        create_ready_current_manifest(&mut connection, 4);
        insert_active_document_catalog(&connection);
        for sql in [
            "INSERT INTO briskdb_document_index_identities VALUES (2, 11, '_id_')",
            "INSERT INTO briskdb_document_index_identities VALUES (1, 11, 'absent')",
            "INSERT INTO briskdb_document_index_identities VALUES (2, 11, 'absent')",
        ] {
        ...
        connection
            ...
            .unwrap();
        assert!(index_identity_rows(&connection).is_empty());
        assert_eq!(
            connection
                .query_row(
    ...
    #[test]
    fn v16_document_lifecycle_upgrade_is_atomic_preserves_ids_and_fences_old_readers() {
        for phase in [
            MigrationPhase::AfterSchemaChange,
            MigrationPhase::AfterVersionStamp,
        ] {
            ...
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
            downgrade_v16_manifest_to_v15_for_test(&connection, 4).unwrap();
            let objects = schema_objects(&connection).unwrap();
            let root = stored_manifest_digest(&connection);
            let error = load_or_create_with_hook(&mut connection, 4, |point| {
                if point.from == V15_SCHEMA_VERSION && point.phase == phase {
                    return Err(EngineError::new(
                        EngineErrorKind::Internal,
                        "injected document lifecycle migration failure",
                    ));
                }
                Ok(())
            })
            ...
            assert_eq!(identity(&connection).1, 15);
            ...
            assert!(startup_requires_exclusive_ownership(&connection, 4).unwrap());
            connection.execute_batch("UPDATE briskdb_document_index_storage SET lifecycle_state = 1, next_shard = shard_count").unwrap();
            refresh_manifest_digest(&connection).unwrap();
            assert!(!startup_requires_exclusive_ownership(&connection, 4).unwrap());
            connection
                .execute(
    ...
    #[test]
    fn document_lifecycle_high_water_and_journal_invariants_are_checksum_authority() {
        for mutation in [
            "DELETE FROM briskdb_document_identities",
            "UPDATE briskdb_document_identities SET database_high_water = 6",
            "UPDATE briskdb_document_identities SET collection_high_water = 10",
            "UPDATE briskdb_document_identities SET collection_high_water = -1",
            "UPDATE briskdb_document_deletion SET database_id = 8",
            "UPDATE briskdb_document_deletion SET collection_id = 12",
            "UPDATE briskdb_document_deletion SET next_shard = 5",
            "UPDATE briskdb_document_deletion SET next_shard = -1",
            "UPDATE briskdb_document_deletion SET shard_count = 3",
            "UPDATE briskdb_document_deletion SET operation_id = zeroblob(31)",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
        ...
        for mutation in [
            "UPDATE briskdb_document_identities SET database_high_water = 100",
            "UPDATE briskdb_document_identities SET collection_high_water = 100",
            "UPDATE briskdb_document_deletion SET collection_id = NULL",
            "UPDATE briskdb_document_deletion SET next_shard = 1",
            "UPDATE briskdb_document_deletion SET operation_id = randomblob(32)",
        ] {
            let mut connection = Connection::open_in_memory().unwrap();
            create_ready_current_manifest(&mut connection, 4);
            insert_active_document_catalog(&connection);
    ...
    fn insert_active_document_catalog(connection: &Connection) {
        connection
            ...
            .unwrap();
        if read_identity(connection).unwrap().1 >= i64::from(V16_SCHEMA_VERSION) {
            connection.execute("UPDATE briskdb_document_identities SET database_high_water = 7, collection_high_water = 11 WHERE singleton = 1", []).unwrap();
        }
        if read_identity(connection).unwrap().1 >= i64::from(V17_SCHEMA_VERSION) {
            connection
                .execute_batch(
                    "INSERT INTO briskdb_document_index_identities VALUES (1, 11, '_id_');
                 UPDATE briskdb_document_index_allocator SET index_high_water = 1;",
                )
                .unwrap();
        }
    ...
    fn document_catalog_cross_row_invariants_fail_closed_after_resealing() {
        ...
        connection
            ...
            .unwrap();
        // Restore the mapping deleted by the foreign-key cascade so this
        // existing test still isolates the provisioning lifecycle invariant.
        connection
            .execute(
                "INSERT INTO briskdb_document_index_identities VALUES (1, 11, '_id_')",
                [],
    ...
    fn document_namespace_limit_is_validated_across_catalog_rows() {
        ...
        connection
            ...
            .unwrap();
        connection.execute("UPDATE briskdb_document_identities SET database_high_water = 7, collection_high_water = 11 WHERE singleton = 1", []).unwrap();
        connection.execute_batch("INSERT INTO briskdb_document_index_identities VALUES (1, 11, '_id_'); UPDATE briskdb_document_index_allocator SET index_high_water = 1;").unwrap();
        refresh_manifest_digest(&connection).unwrap();
        validate_document_catalog(&connection, 4).unwrap();
