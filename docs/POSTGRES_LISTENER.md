...
Two additional parser-bounded client adapters feed that same Engine path:

- exact bare `START TRANSACTION`, used by tested tokio-postgres, becomes
  `BEGIN`; isolation, access-mode, and deferrability clauses are rejected; and
- only an extended-query placeholder with an unparameterized PostgreSQL
  `::VARCHAR` cast, as generated by tested SQLAlchemy DML, has that cast
  removed before common-subset validation.

Literal casts, `CAST(...)`, other target types, parameterized `VARCHAR(n)`, and
statement batches are not broadened by these adapters. See the
[release-gating client matrix](POSTGRES_CLIENTS.md).
...
- live psql, tokio-postgres, psycopg, and SQLAlchemy ORM transaction CRUD,
  unsupported-query recovery, clean close, and reconnect;
