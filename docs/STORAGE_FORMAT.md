...
table provisioning, hi/lo leasing, and global-index state. The v15-to-v16
transaction adds document identity high-water marks and a deletion journal,
installs the version-16 fence, and advances semantic manifest digest to version 8.
High-water marks begin at the existing maximum database and collection IDs
(zero for empty catalogs). The upgrade changes no shard file or application row.
The v16-to-v17 step adds stable index identities and a permanent allocation head,
installs the version-17 fence, and advances the semantic digest to version 9.
The v17-to-v18 step adds a checksummed physical document-index layout journal,
installs the version-18 fence, and advances the semantic digest to version 10.
Sole-process startup then provisions empty entry tables on document shards;
existing BSON and index declarations/IDs are unchanged, with no index activation.
The v18-to-v19 step adds an empty checksummed index-operation journal, installs
the version-19 fence and advances the semantic digest to version 11. The upgrade
does not build any index; explicit non-unique builds can subsequently publish Ready.
The v19-to-v20 step adds no tables or entry formats: it installs the version-20
...
CREATE TABLE briskdb_document_identities (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
...
CREATE TABLE briskdb_document_deletion (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    database_id INTEGER NOT NULL CHECK (database_id > 0),
...
CREATE TABLE briskdb_document_index_storage (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
...
Version 18 supports only the `active` physical-shard lifecycle state. Adding
...
Every fresh or upgraded v18 manifest contains SQL logical database ID `1` named
...
identifier encoding version `1` and default database ID `1`. Version 18 permits
...
Document identity high-water marks must be at least the greatest existing IDs
and are never lowered by supported mutations. Allocation increments one mark
in the catalog creation transaction, failing at signed-64-bit exhaustion. The
deletion journal references an existing database and, when non-null, an active
collection belonging to it. Null collection ID selects all collections in that
database. Journal shard count equals the immutable root count, and provisioning
and deletion may not coexist. These relationships and every progress field are
covered by digest version 8.

Under sole-process schema ownership, deletion commits intent, deletes the exact
target IDs on each shard (or removes the exact optional document table when no
collections remain), and acknowledges each committed shard in a sealed manifest
transaction. Finalization removes index/collection metadata, the empty database,
and the journal together. Interrupted accepted intent fences the root until
startup rolls forward its idempotent remaining prefix. SQL objects and unrelated
document namespaces are not deletion targets. See the
[recovery contract](DOCUMENT_STORAGE.md#namespace-deletion-and-restart).
...
Fresh version 17 storage seeds active `owner_slot = physical_shard_id`, so the
...
count or bucket map. Version 18 has no public owner-map mutation operation, but
...
a v9 catalog migrated with an inactive native policy: v18 may provision that
...
Fresh v18 initialization leaves the SQL table, generated-ID, hi/lo,
generated-table DDL, global-index, document, and provisioning catalogs empty.
The document identity singleton starts with both high-water marks at zero.
...
manifests have an empty journal. Fresh v18 initialization and pre-v6 upgrades
...
| `briskdb_document_identities` | `singleton`, `database_high_water`, `collection_high_water` |
| `briskdb_document_deletion` | `singleton`, `database_id`, `collection_id`, `operation_id`, `shard_count`, `next_shard` |
| `briskdb_document_index_identities` | `index_id`, `collection_id`, `index_name` |
...
journal. Digest version 9 remains frozen solely to verify and migrate version-17
manifests. Its `briskdb.manifest.semantic-root.v9` domain and stream omit the
index-storage layout journal. Digest version 8 remains frozen solely to verify and migrate version-16
manifests. Its `briskdb.manifest.semantic-root.v8` domain and stream omit the
two index identity tables. Digest version 7 remains frozen solely to verify and migrate version-14 and
version-15 manifests. It uses the `briskdb.manifest.semantic-root.v7` domain and
the same encoding but omits document identity high-water marks and deletion.
...
two version-9 tables. A v18 manifest must store version 10; storing an older
digest in an otherwise v18 shape is corruption, and an unsupported future
...
[canonical global-index key format](INDEX_KEY_ENCODING.md). Version 18 records
...
`schema_generation`. Version 18 accepts only routing generation 1 and validates
...
Version 19 has the same manifest objects and physical record/entry formats as
version 20, but only authorizes non-unique Ready secondary indexes. Its semantic
digest is version 11. The atomic v19-to-v20 migration replaces the exact metadata
fence with `requires_manifest_version >= 20`, stamps version 20, and reseals the
unchanged authoritative rows with digest version 12 and its new domain. Both
failure sides roll back to exact v19; frozen v19 readers reject the upgraded root
before writing. Unique activation must never be backported without this fence.

## Previous version 18

Version 18 has twenty-eight strict manifest tables and semantic digest version 10.
The atomic upgrade adds this empty table, advances the fence/header to 19 and
reseals with digest version 11; older readers fail before writing the root.
...
PendingBuild secondary index in an active collection. Version 19 requires
non-unique Build/Abort targets and rejects Ready secondary unique indexes.
Version 20 permits both, with unchanged entry schema and journal kinds. The exact root shard
...
collection ID and index ID as u64 LE, shard as u16 LE, u64 LE length plus canonical
ID bytes, u64 LE length plus BDIK tuple bytes, and the current record checksum's
...
uniqueness with cross-process collection-writer fences, validated cross-shard
key probes before record mutation, and disk-backed global key validation during
offline builds/startup. Same-owner multikey repetitions are not duplicates.
Record/entry changes commit together; whole-bulk post-image uniqueness and
cross-shard atomic writes remain unimplemented. See the
[engine enforcement boundary](DOCUMENT_ENGINE.md#equality-index-candidates).

## Previous version 17

Version 17 has twenty-seven strict manifest tables and semantic digest version 9.
The atomic v17-to-v18 manifest transaction adds `briskdb_document_index_storage`,
installs the version-18 fence/header, and reseals with digest version 10. The
singleton uses entry format 1 and the exact root shard count. A root with any
collection starts in `Upgrading` (2), cursor 0; an empty root starts `Ready` (1),
cursor equal to shard count. It preserves all existing semantic rows and BSON.

The pending layout requires sole-process startup ownership. Namespace deletion
or provisioning recovery runs first. Each remaining document shard then creates
the following exact schema in one transaction, followed by a separate sealed
manifest cursor commit. A crash between commits safely retries the shard.
Only after the complete shard prefix is durable does a final manifest transaction
mark the layout Ready. The format version itself identifies this one-time layout
upgrade; the journal is retained after completion. Empty catalogs create no
physical tables. Collection provisioning subsequently creates both record and
entry schemas together; dropping the last namespace removes both.

```sql
CREATE TABLE briskdb_document_index_entries_v1 (
...
CREATE INDEX briskdb_document_index_entries_by_record_v1
    ON briskdb_document_index_entries_v1 (collection_id, id_key, index_id);
```

These reserved objects are excluded from application-schema hashes and ordinary
SQL access. Version 18 required them to
be empty and rejected nonempty, orphaned or malformed storage. Missing entry
schema in a Ready root is corruption, not an automatic rebuild. Secondary
declarations stay PendingBuild during this upgrade; no existing index is populated,
enforced or used by the planner. Version 19 adds the entry checksum, maintenance
and explicit build authority described above. Downgrading is unsupported;
version-17 and older readers reject the fence without modifying the root.

## Previous version 16

Version 16 has twenty-five strict manifest tables and semantic digest version 8.
The atomic v16-to-v17 transaction adds `briskdb_document_index_identities` and
`briskdb_document_index_allocator`, installs the version-17 fence/header, and
reseals with digest version 9. Existing indexes receive positive IDs in ascending
`(collection_id, index_name COLLATE BINARY)` order. The allocator starts at the
highest assigned ID, or zero for an empty catalog. Index specifications remain
byte-for-byte unchanged, including opaque pending legacy/import metadata; flags,
lifecycles, collection IDs, shard files and application rows are untouched.

Each new built-in or secondary declaration allocates an ID in the same manifest
transaction as its insertion. Idempotent redeclarations retain the existing ID.
Mappings have a unique `(collection_id, index_name)` pair and a cascading foreign
key to the declaration. Drops remove mappings but never lower the high-water mark,
even after the final namespace disappears. Allocation rejects signed-64-bit
exhaustion without partial catalog changes. Startup validates exact one-to-one
coverage, positive IDs and allocator bounds; both tables are checksum authority.
IDs are independent of the SQL-index identity space and do not activate physical
indexes. Version-16 and older binaries refuse this upgraded root.
...
Version 15 has twenty-three strict manifest tables, semantic digest version 7,
and the version-15 fence. It recognizes optional shard-local idempotency receipts
but has no durable document deletion intent or permanent catalog identity marks.
The atomic v15-to-v16 migration adds those two tables, seeds the singleton marks
from the existing catalog maxima, leaves deletion empty, installs the version-16
fence/header, and reseals with digest version 8. All prior catalog, routing,
migration, allocator and shard state is preserved. Old readers refuse the new
header/fence before they can ignore an interrupted drop or recycle identities.
...
v12-to-v13, v13-to-v14, v14-to-v15, v15-to-v16, v16-to-v17, and v17-to-v18 transactions.
...
5. Apply each compile-time registered manifest migration with static SQL and
   ...
   the digest version or manifest object set; and v15-to-v16 adds permanent
...
6. Fresh initialization is allowed only beside an otherwise empty physical
   layout and commits v18 physical state `Creating`, integrity state
   ...
   catalog; v14-to-v15 installs the receipt-format fence; and v15-to-v16 adds
   identity high-water marks and an empty deletion journal.
   The v16-to-v17 step adds index identities and their permanent allocation head.
   The v17-to-v18 step adds an empty Ready index-layout journal.
7. If the validated integrity state is `Degraded`, fail startup without
   changing it. Otherwise, if a validated v18 manifest contains one `Applying`
...
12. Before readiness, recover any document deletion or provisioning journal under
    sole-process ownership, then validate the active document catalog and shard
    records. A build without documents refuses collection/deletion authority.
    In one manifest transaction, seal a first consensus as the committed
...
`manifest.sqlite`. A version-1 upgrade commits versions 2 through 18; a
...
The v15-to-v16 step changes no shard: it adds permanent document identity marks
initialized to catalog maxima and an empty deletion journal, raises the fence,
and moves the checksum to version 8. Deletion occurs only after an explicit
namespace drop commits intent, not as an upgrade side effect.
...
v1/v2/v3/v4/v5/v6/v7/v8/v9/v10/v11/v12/v13/v14/v15/v16/v17 upgrade path to v18, every
...
Document lifecycle tests cover atomic v15-to-v16 upgrade/rollback and old-reader
refusal, high-water exhaustion, non-reused cursor identities, checksum and
relational corruption, and 80 subprocess exits around deletion commit boundaries.
Index identity tests cover atomic v16-to-v17 migration, exact legacy bytes,
old-reader refusal, checksum and coverage corruption, idempotency, exhaustion
rollback, and non-reuse across namespace deletion/recreation and those crash exits.
Cancellation, deadline, and task-abort tests fence accepted intent and recover
it on reopen. Document catalog and storage tests cover the v13-to-v14 migration, semantic
