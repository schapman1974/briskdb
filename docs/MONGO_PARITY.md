...
ping, inspect build information, insert one or many documents, and find a literal
exact `_id`. Missing IDs are generated by the shared engine when the client has
...
General filters, projection, sorting, updates, deletes, and retained cursors
