...
find queries through the [shared BSON matcher](DOCUMENT_ENGINE.md).
Exact `_id` and `_id: {$eq: value}` filters keep single-shard routing;
other filters scan and merge matching documents in durable natural order.
...
| Find `batchSize` | Integer from 1 through 1000; results must fit one batch until retained cursors land |
...
Projection, sorting, updates, deletes, and retained cursors
...
driver batch splitting, concurrent writes, reconnection, resource limits, and
